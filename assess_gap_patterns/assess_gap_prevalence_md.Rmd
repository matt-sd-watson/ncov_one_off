---
title: "Assessing Gap Prevalence in COVID-19 samples"
author: "BCC PHO"
date: "`r format(Sys.time(), '%B %d, %Y')`<div style=\"height: 40px;\"></div>"
output:
  html_document:
    toc: yes
    toc_float: no
...
## Gap distance Distributions
```{r, echo=F, warning=F, message=F}
file_path <- "/home/mwatson/COVID-19/master_fasta/"

find_nextclade <- list.files(file_path, pattern = "nextclade_all*")
nextclade_all <- read.csv(paste(file_path, find_nextclade, sep=""), header = TRUE, na.strings=c("","NA"), stringsAsFactors=FALSE,
                          sep="\t")
...
## Gap density for Accepted Gisaid sequences

```{r, echo=F, warning=F, message=F}
gisaid_names_path <- "input/"

find_gisaid_names <- list.files(gisaid_names_path, pattern = "all_gisaid*")
# read a CSV file containing all of the WGS Ids that have been Gisaid accepted
all_gisaid_accepted <- read.csv(paste(gisaid_names_path,
                                      find_gisaid_names,
...
## Gap frequencies based on Gisaid submittion (Yes/No)
...
## Summary statiustics for gaps based on Gisaid submission (Yes/No)
...
```{r, echo=F, warning=F, message=F}
datatable(tidy_gaps %>% filter(distance != "None", !grepl("1-5|-29903", gap)) %>% group_by(seqName, category) %>% summarise(num_gaps = n()) %>% group_by(category) %>%
...
```{r, echo=F, warning=F, message=F}
datatable(tidy_gaps %>% filter(distance != "None", !grepl("1-5|-29903", gap)) %>% group_by(seqName, category) %>%
...
```{r, echo=F, warning=F, message=F}
datatable(tidy_gaps %>% filter(distance != "None", !grepl("1-5|-29903", gap)) %>% group_by(category) %>%
...
```{r, echo=F, warning=F, message=F, fig.width = 12, fig.height = 9}
...
# identify gaps
data_for_plot <- data_for_plot[!grepl("NA|1-5|-29903", data_for_plot)]
...
# Gap prevalence split by sequence length bins

\
\
```{r, echo=F, warning=F, message=F}
find_length_list <- list.files(gisaid_names_path, pattern = "sample_lengths*")

length_file <- read.csv(paste(gisaid_names_path,
                              find_length_list,
                              sep=""), header = FALSE, na.strings=c("","NA"),
                        stringsAsFactors=FALSE)

lengths_processed <- data.frame(sample = str_split_fixed(length_file$V1,
                                                         "Sequence: ", 2)[,2],
                                length = as.numeric(
                                  str_split_fixed(length_file$V2, 
                                                  "Length: ", 2)[,2]))

pho_lengths <- lengths_processed %>% filter(!grepl(
...
for (i in unique(tidy_gaps_with_length$cat)) {
  subset_length <- subset(tidy_gaps_with_length, cat == i) %>% filter(!grepl("NA|1-5|-29903",
                                                                  gap))
...
lineage_file <- list.files(file_path, pattern = "*plearn*")

lineages <- read.csv(paste(file_path,
                           lineage_file,
...
gaps_lin_freqs <- tidy_gaps_with_lineages[!duplicated(tidy_gaps_with_lineages[1]),] %>%
  ...
  filter(counts > 300) %>% mutate(lineage = ifelse(grepl("AY", lineage),
                                                   "B.1.617.2", lineage))
...
# Summary Statistics for gaps based on sequence length
<br>

## Median sequence length for VOCs


```{r, echo=F, warning=F, message=F}

datatable(tidy_gaps_with_lineages[!duplicated(tidy_gaps_with_lineages$seqName),] %>% group_by(lineage) %>% summarise(avg = median(length)) %>%
...
## Mean number of gaps by sequence bin

```{r, echo=F, warning=F, message=F}

datatable(tidy_gaps_with_length %>% filter(distance != "None", !grepl("1-54|29837-29903", gap)) %>% group_by(seqName, cat) %>%
...
## Mean gap length by sequence bin
...
## Median gap length by sequence bin

```{r, echo=F, warning=F, message=F}

datatable(tidy_gaps_with_length %>% filter(distance != "None", !grepl("1-54|29837-29903", gap)) %>% group_by(cat) %>%
